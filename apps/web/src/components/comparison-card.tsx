...
export function ComparisonCard({
  ...
  // Use the useChat hook with oRPC transport for comparison endpoint
  const { messages, sendMessage, status, error } = useChat({
    transport: {
      async sendMessages(options) {
        const result = await client.compareChat(
          {
            messages: options.messages,
            selectedPartyIds: parties.map((p) => p.shortName),
            originalQuestion: messageTrigger?.message,
          },
    ...
    <div className={cn('w-full space-y-4', className)}>
        ...
        <div className="pb-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="flex size-10 items-center justify-center rounded-full bg-gradient-to-r from-blue-500 to-purple-600 text-white">
      ...
      {error && (
        <div className="rounded-md bg-destructive/10 p-3 text-destructive text-sm">
          Feil ved sammenligning: {error.message || 'Ukjent feil'}
        </div>
      )}
      ...
      {!(hasMessages || isLoading || error) && (
        <div className="py-8 text-center">
          <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-r from-blue-500 to-purple-600 text-white text-xl">
            <BarChart3Icon className="size-6" />
          </div>
          <h3 className="mb-2 font-medium text-lg">Partisammenligning</h3>
          <p className="text-muted-foreground text-sm">
            Still et spørsmål for å få en sammenligning av partiene
          </p>
                  ...
                  if (part.type === 'text') {
                    return (
                      <div key={`${message.id}-${partIndex}`}>
                        <Response partyShortName="sammenligning">
                          {part.text}
                        </Response>
                      </div>
                    );
